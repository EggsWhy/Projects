...
def queue_music():
    ...
    global current_song, songs, MUSIC_END, current_time
...
edit_menu.add_command(label='New',command=new)
...
volume_scale = tk.Scale(window, from_=100, to=0, tickinterval=10, length=200, orient='horizontal', command=adjust_volume)
volume_scale.pack()
...
# window.bind('<Left>',previous_music)
# window.bind('<Right>',next_music)
window.bind('<space>',play_music_shortcut)
window.bind('<k>',pause_music_shortcut)
